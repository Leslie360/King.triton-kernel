...
class Settings(BaseSettings):
    ...
    worker_monitor_task_progress_timeout: int = Field(
        default=180,
        env="WORKER_MONITOR_TASK_PROGRESS_TIMEOUT",
        description=(
            "Seconds a worker may stay on one task with no progress (tasks_processed unchanged) "
            "while reporting online before the monitor treats it as a wedged GPU and restarts it. "
            "Must exceed the normal per-task ceiling (~35s/attempt x 3 retries ~ 105s)."
        ),
    )
...
TASK_CONFIGS = {
    ...
    "standard": {"num_correct_trials": 3, "num_perf_trials": 100, "timeout": 300, "priority": "normal"},
