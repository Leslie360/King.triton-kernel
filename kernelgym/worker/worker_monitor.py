...
class WorkerMonitor:
    ...
    def __init__(self, redis_client: redis.Redis, persistent: bool = False):
        ...
        # Wedged-GPU safety net: a worker whose processing loop is stuck (e.g. a
        # subprocess blocked in an uninterruptible CUDA op, or an orphaned CUDA
        # context pinning the GPU at 100%) still sends heartbeats from the
        # independent heartbeat loop, so the heartbeat-timeout check above never
        # fires. We detect this by watching tasks_processed: while a worker is
        # genuinely on a task (current_task set) it must finish within ~35s/attempt
    ...
    async def _check_workers(self):
        ...
        try:
            ...
            for key in worker_keys:
                ...
                # whose processing loop is stuck keeps heartbeating (separate
                # asyncio task) so tasks_processed flatlines while current_task
                # stays set -- that is the wedge signature.
                try:
                    tasks_processed = int(worker_info.get("tasks_processed") or 0)
                except (TypeError, ValueError):
                    tasks_processed = 0
                current_task = worker_info.get("current_task", "")
                online = worker_info.get("online") == "true"
                prev_state = self.monitored_workers.get(worker_id, {})
                prev_tps = prev_state.get("last_tasks_processed")
                prev_stale_since = prev_state.get("progress_stale_since")
                if online and current_task and prev_tps is not None and tasks_processed == prev_tps:
                    # On a task but tasks_processed unchanged since last check
                    progress_stale_since = prev_stale_since or datetime.now()
                else:
                    progress_stale_since = None
                ...
                # The heartbeat loop is a separate asyncio task, so a worker
                # whose processing loop is stuck (subprocess blocked in an
                # uninterruptible CUDA op, or an orphaned CUDA context pinning
                # the GPU at 100%) still heartbeats and would otherwise never
                # be restarted. Restart tears down the whole process group +
                # resets the GPU to reclaim the orphaned context.
                if not needs_restart and progress_stale_since is not None:
                    stale_secs = (datetime.now() - progress_stale_since).total_seconds()
                    if stale_secs > self.task_progress_timeout:
                        needs_restart = True
                        restart_reason = (
                            f"No task progress in {int(stale_secs)}s while on task "
                            f"'{current_task}' (wedged GPU / stuck subprocess)"
                        )
                ...
                self.monitored_workers[worker_id] = {
                    ...
                    "last_tasks_processed": tasks_processed,
                    "progress_stale_since": progress_stale_since,
                }
