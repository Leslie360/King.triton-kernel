...
class GPUWorker:
    ...
    def __init__(self, worker_id: str, device: str, redis_client: redis.Redis):
        ...
        self.worker_pool: SubprocessWorkerPool | None = None
                ...
                try:
                    ...
                    async with self.http_session.post(url, params={"hostname": hostname}) as resp:
                        if resp.status == 200:
                            ...
                            logger.info(
                                f"Obtained server-assigned node_id={self.node_id} for hostname={hostname}"
                            )
                        else:
                ...
                try:
                    async with self.http_session.post(url, params=params) as response:
                        if response.status == 200:
                            ...
                            logger.error(
                                f"Failed to register with API server: {response.status} - {error_text}"
                            )
...
async def main():
    ...
    if available_devices < required_devices:
        logger.error(
            f"Not enough GPUs available. Required: {required_devices}, Available: {available_devices}"
        )
        sys.exit(1)
